...
## ✅ SESIÓN XX+81 (2026-05-12) — Firma digital incidentes en mobile padre + documentación

**Fecha:** 2026-05-12 | **Estado:** ✅ Completado

### Completado

- **Nuevo componente:** `mobile/src/components/SignaturePadMobile.jsx` — canvas de firma táctil implementado con `react-native-svg` + `react-native-gesture-handler` (Expo Go compatible, sin WebView ni dependencias nativas extra)
- **Mobile padre bitácora:** `(padre)/bitacora.jsx` — tab Incidentes ahora muestra botón azul "✍️ Firmar incidente" en lugar del mensaje "revisa en el portal web". Al tocar → modal bottom-sheet con canvas táctil → firma con dedo → mutation `PATCH /bitacora/incidente/:id/firma` con `firma_data` (SVG base64) → badge verde ✅ Firmado
- **Paridad cerrada:** Firma digital de incidentes ahora disponible en web (padre/Bitacora.jsx canvas) Y mobile (SignaturePadMobile SVG) ✅
- **ARCHITECTURE.md:** Tabla paridad actualizada (fila Firma digital → ✅ Paridad), gap removido, `(padre)/bitacora.jsx` documenta SignaturePadMobile
- **architecture.html:** Mismos cambios en tabla HTML, fila firma reubicada, descripción pantalla actualizada
- **05_manual_padre.md:** Sección "Firmar incidente" separada en flujo mobile (6 pasos) + flujo web (5 pasos)
- **manual_padre.html:** Mismos cambios con dos listas de pasos numerados

### Archivos modificados

- `mobile/src/components/SignaturePadMobile.jsx` (NUEVO)
- `mobile/app/(padre)/bitacora.jsx`
- `docs/v1.0.0-beta/ARCHITECTURE.md`
- `docs/v1.0.0-beta/architecture.html`
- `docs/v1.0.0-beta/05_manual_padre.md`
- `docs/v1.0.0-beta/manual_padre.html`

### Decisiones técnicas

- Se descartó `react-native-signature-canvas` (requiere `react-native-webview` + build nativo)
- Implementación propia con SVG paths (M/L commands) + GestureHandler Pan — funciona en Expo Go sin rebuild
- La firma se guarda como SVG base64 en `firma_padre_url` — mismo campo que usa la web

---

## ✅ SESIÓN XX+79 (2026-05-12) — FiltroEntrada: checklist default true + fix comida cancelada web+mobile

**Fecha:** 2026-05-12 | **Estado:** ✅ Completado

### Completado

- **Bug fix web:** Sección Comida en FiltroEntrada aparecía aunque la directora ya hubiera cancelado el servicio (`estado = 'cancelado'`). Fix: condición `confirmacionComida.estado !== 'cancelado'` en render y en `handleSubmit`
- **UX web:** `CHECKS_DEFAULT` en `FiltroEntrada.jsx` — todos los ítems cambiados a `true` por default (uñas_cortadas, sin_lagañas, panial_limpio, trajo_paniales, trae_uniforme, trae_bata, trae_termo, agua_suficiente). La miss desmarca solo lo que el niño NO trajo — flujo más rápido
- **Paridad mobile:** `qr-scanner.jsx` (`ChecklistEntrada`) ya tenía defaults en `true` ✅
- **Paridad mobile pañal:** Agregados `panial_limpio` y `trajo_paniales` condicionales (`alumno.usa_panial`) en checklist mobile — faltaban vs web
- **Paridad mobile comida:** Agregado `useEffect` para cargar confirmación de comida + botón con mismo fix `estado !== 'cancelado'` — faltaba vs web
- `ChecklistEntrada` mobile convertido a `ScrollView` para soportar todos los campos en pantallas pequeñas
...
## ✅ SESIÓN XX+78 (2026-05-11) — Validación sin_recoger + Fix extensión horario job automático

**Fecha:** 2026-05-11 | **Estado:** ✅ Completado

### Completado

- Validado botón 📲 en FiltroSalida: WA llegó a 9932160007 (Camila y Lucía Jiménez Vargas)
- Detectado bug: job automático `sinRecogerJob` alertaba a alumnos con extensión de horario a las 15:05, sin respetar que su salida real es 18:00
- Fix `sinRecogerJob.js`: query enriquecida con `LEFT JOIN config_horario_alumno` trae `tiene_extension` y `hora_salida_extension` por alumno
- Lógica de filtro: cada alumno comparado contra su `hora_salida_extension` (si aplica) + minutos de gracia — no contra `hora_salida_normal` global
- Mensaje WA/notificación muestra la hora correcta del alumno ("18:00" para extensión, "15:00" para normal)
- Cron ampliado de `14-17` a `14-19` para cubrir alertas de alumnos con extensión hasta 18:00
- Backend reiniciado y validado: `✅ Job sin_recoger configurado: cada minuto, 14:00-19:00, lunes-viernes (México)`

### Archivos modificados

- `backend/src/jobs/sinRecogerJob.js`
- `PENDIENTES.md` (sin_recoger marcado como [x])
...
Sesión de validación end-to-end de todas las plantillas WhatsApp con Twilio Sandbox. Se descubrió y corrigió un patrón de bug recurrente: varios flujos llamaban `enviarPush` sin hacer `INSERT INTO notificaciones`, por lo que las alertas nunca aparecían en la campanita ni en el modal del portal web. Se corrigieron `fiebre`, `recordatorio_pago` y `persona_no_autorizada`. También se implementó el job automático de `sin_recoger` y se mejoró el texto de la plantilla `recordatorio_pago`.

### Validaciones completadas

| Plantilla | Estado |
|-----------|--------|
| `fiebre` | ✅ WA validado + fix campanita web (INSERT notificaciones) |
| `medicamento` | ✅ WA validado |
| `incidente` | ✅ WA validado |
| `alerta_salud` | ✅ WA validado |
| `recordatorio_pago` | ✅ WA validado + modal web validado (fix INSERT notificaciones) |
| `salida_anticipada` | ✅ WA validado |
| `recargo` | ✅ descartado — mismo flujo que recordatorio_pago |

### Fixes aplicados

| Fix | Archivo(s) |
|-----|-----------|
| Fix fiebre: `INSERT INTO notificaciones` antes de `enviarPush` — campanita web ahora funciona | `backend/src/routes/bitacora.js` |
| Fix fiebre: query `padresFiebre` sin `usuario_id` — agregado al SELECT | `backend/src/routes/bitacora.js` |
| Fix recordatorio_pago: subconsulta con `al.id` no agrupado → reemplazada por `MIN(p.fecha_limite)` | `backend/src/routes/pagos.js` |
| Fix recordatorio_pago: agregado `INSERT INTO notificaciones` + `enviarPush` + `usuario_id` en query | `backend/src/routes/pagos.js` |
| Fix persona_no_autorizada: agregado `usuario_id` al query tutor + `INSERT notificaciones` + `enviarPush` | `backend/src/routes/asistencia.js` |
| Plantilla `recordatorio_pago` actualizada: texto con fecha actual y vencimiento día 5 | BD `plantillas_whatsapp` |
| Nuevo job `sinRecogerJob.js`: cron cada minuto 14:00-17:00 lunes-viernes, detecta alumnos sin salida y notifica WA + modal + push | `backend/src/jobs/sinRecogerJob.js`, `backend/src/index.js` |
| `NotificacionModal.jsx`: agregados tipos `sin_recoger` (naranja ⏰) y `persona_no_autorizada` (rojo 🚨) | `web/src/components/NotificacionModal.jsx` |
| BD `notificaciones_modal_tipos`: agregados `sin_recoger` y `persona_no_autorizada` | `configuracion_general` |

### Bug histórico documentado — NUNCA REPETIR

**BUG #20 — enviarPush sin INSERT notificaciones:**
Cuando se agrega una notificación nueva, siempre hacer `INSERT INTO notificaciones` ANTES de llamar `enviarPush`. El push solo llega a mobile (Expo token). La campanita y modal web leen de la tabla `notificaciones` en BD. Sin el INSERT, el padre nunca ve nada en web.

---

## ✅ SESIÓN XX+75 (2026-05-11) — Plantillas WhatsApp finales + validación Twilio retardo

**Fecha:** 2026-05-11 | **Estado:** ✅ Completado

### Resumen

Auditoría y reestructuración final de plantillas WhatsApp. Quedaron 12 plantillas activas con disparadores reales. Se conectaron 2 plantillas nuevas (`fiebre`, `sin_comida`), se agregó endpoint y botón para aviso de recargo desde día 6, se limpió `bitacora_lista` (plantilla eliminada que generaba fallos silenciosos), y se validó end-to-end el envío de `retardo` con Twilio Sandbox.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| Migración 052: elimina `no_entrada` y `suspension`, actualiza texto `sin_recoger` con aviso de cobro, inserta `fiebre` y `sin_comida`, activa 12 plantillas finales | `backend/migrations/052_plantillas_whatsapp_final.sql` |
| Nuevas funciones `notificarFiebre()` y `notificarSinComida()`; eliminadas funciones de plantillas ya borradas | `backend/src/services/whatsappService.js` |
| Disparador automático `fiebre` al guardar bitácora con `tuvo_fiebre=true` (1 vez por día); quitado envío WA de `bitacora_lista` (plantilla eliminada) | `backend/src/routes/bitacora.js` |
| Nuevo endpoint `POST /pagos/alumno/:id/aviso-recargo` (solo disponible desde día 6 del mes) | `backend/src/routes/pagos.js` |
| Botón ⚠️ en tabla Adeudos visible cuando `recargo_pendiente > 0` y día >= 6 | `web/src/pages/directora/Pagos.jsx` |
| Guarda centralizada: teléfono vacío/null → `omitido: true` sin log de error | `backend/src/services/whatsappService.js` |
| Fix: revertido cambio erróneo de normalización — confirmado que `+521XXXXXXXXXX` es el formato correcto para sandbox Twilio | `backend/src/services/whatsappService.js` |

### Plantillas activas finales (12)

| Clave | Disparador |
|-------|-----------|
| `retardo` | Automático al registrar retardo — ✅ validado Twilio |
| `sin_recoger` | Botón 📲 en FiltroSalida después de las 15:00 |
| `persona_no_autorizada` | Automático al detectar persona no autorizada |
| `salida_anticipada` | Automático al registrar salida anticipada |
| `alerta_salud` | Automático en bitácora vómito |
| `medicamento` | Automático al registrar medicamento |
| `incidente` | Automático al registrar incidente |
| `fiebre` | Automático al guardar bitácora con fiebre (nuevo) |
| `recordatorio_pago` | Botón 📲 en tabla Adeudos |
| `recargo` | Botón ⚠️ en tabla Adeudos desde día 6 (nuevo) |
| `sin_comida` | Disparador pendiente de conectar a flujo comida (nuevo) |
| `pago_comida_lunes` | Disparador pendiente de conectar (job futuro) |

### Sandbox Twilio
- Código join: `join although-previous` → +14155238886
- Números registrados: 9931669869 (Héctor) y 9932160007 (Adriana)
- Formato correcto: `+521XXXXXXXXXX` (con 1 intermedio)
- ✅ `retardo` validado end-to-end

---

## ✅ SESIÓN XX+74 (2026-05-08) — Documentación v1.0.0-beta: 7 HTMLs generados estilo Mintlify

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Generación de los 7 archivos HTML de documentación estilo Mintlify para la versión v1.0.0-beta. Cada HTML es autocontenido (CSS y JS inlineados, sin dependencias externas). Se usó la estrategia de placeholders amarillos para las capturas de pantalla pendientes, con el nombre exacto de archivo y descripción de qué capturar en cada placeholder.

### Decisiones de sesión

- **Estrategia de imágenes:** Placeholders amarillos (`<div class="img-placeholder">`) en lugar de requerir capturas previas. Valeria puede agregar imágenes después poniendo los `.png` en `assets/img/` con el nombre indicado.
- **Diseño diferenciado por audiencia:** Cada HTML usa un color de acento distinto (Directora: morado, Maestra: verde, Padre: naranja, Admin: azul, Capacitación: ámbar, Soporte: índigo, Técnico: cian)
- **Sidebar fijo + scroll-spy JS:** Resalta automáticamente la sección activa al hacer scroll
- **Checklists interactivos:** `<input type="checkbox">` con `accent-color` en guia_capacitacion.html y guia_tecnica.html
- **FAQ como accordions:** `<details>/<summary>` en faq_soporte.html y secciones de preguntas frecuentes
- **Código técnico dark-themed:** Bloques `<pre><code>` con fondo #161b22 en guia_tecnica.html

### Archivos generados en `docs/v1.0.0-beta/`

| Archivo | Tamaño | Audiencia | Acento |
|---------|--------|-----------|--------|
| `manual_directora.html` | 36 KB | Directora | Morado |
| `manual_maestra.html` | 28 KB | Maestras | Verde |
| `manual_padre.html` | 32 KB | Padres/Tutores | Naranja |
| `manual_administrador.html` | 24 KB | Administrativo | Azul |
| `guia_capacitacion.html` | 48 KB | Valeria (alta + onboarding) | Ámbar |
| `faq_soporte.html` | 32 KB | Valeria (soporte piloto) | Índigo |
| `guia_tecnica.html` | 44 KB | Valeria (vars + migs + respaldo + rollback) | Cian |

### Pendiente para próxima sesión

- Tomar capturas de pantalla reales (23 imágenes en total identificadas en PENDIENTES.md)
- Reemplazar placeholders por `<img>` reales en los 7 HTMLs

---

## ✅ SESIÓN XX+73 (2026-05-08) — Documentación v1.0.0-beta: 11 documentos .md redactados

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Sesión de documentación completa para el piloto con Happys School. Se redactaron los 11 documentos de la sección "📚 DOCUMENTACIÓN — Pre-QA / Piloto con escuela real" de PENDIENTES. Se declaró la versión como **v1.0.0-beta** (pendiente deploy + APK nativo + Twilio producción para llamarla v1.0.0 final).

### Decisiones de sesión

- **v1.0.0-beta** (no v1.0.0): deploy, APK nativo y Twilio producción son bloqueantes para v1.0.0 final
- `docs/` limpiada: 5 archivos de desarrollo obsoletos eliminados
- Estructura `docs/v1.0.0-beta/` creada con 11 documentos .md + README índice
- HTMLs pendientes para próxima sesión (requieren capturas de pantalla reales de la app)

### Documentos creados en `docs/v1.0.0-beta/`

| Archivo | Tipo | Contenido |
|---------|------|-----------|
| `README.md` | Índice | Índice general de toda la documentación |
| `01_checklist_alta_escuela.md` | Capacitación | Orden exacto para dar de alta una escuela (12 pasos con dependencias FK) |
| `02_guia_onboarding.md` | Capacitación | Guía detallada módulo a módulo para primera configuración |
| `03_manual_directora.md` | Manual usuario | 16 módulos: Dashboard, Alumnos, Grupos, Personal, Usuarios, Pagos, Bitácora, Calendario, Niños Extensión, Visitantes, Aviso, Ciclos, Comida, Config, Catálogos, Turno Puerta |
| `04_manual_maestra.md` | Manual usuario | Web + Mobile: Filtro Entrada, Filtro Salida, Asistencia, Bitácora, Tareas, QR Scanner |
| `05_manual_padre.md` | Manual usuario | Web + Mobile: Dashboard, Bitácora, Calendario, Comida, Pagos, QR (permanente + temporal) |
| `06_manual_administrador.md` | Manual usuario | Dashboard financiero, Pagos, Notificaciones de adeudo, Comida pagos |
| `07_faq_soporte.md` | Soporte | 8 secciones: acceso, QR, entrada/salida, personas autorizadas, pagos, bitácora, app mobile, configuración |
| `08_variables_entorno.md` | Técnico | 27 vars backend + 1 web + 1 mobile, con notas de Railway/Vercel |
| `09_guia_migraciones.md` | Técnico | 49 migraciones, comandos node-pg-migrate, primer deploy y actualizaciones |
| `10_plan_respaldo.md` | Técnico | pg_dump, frecuencia, restauración, backup automático Railway |
| `11_plan_rollback.md` | Técnico | 5 escenarios de fallo con pasos exactos, rollback Railway/Vercel |

### Pendiente para próxima sesión

- Capturas de pantalla reales (levantar servidores → tomar screenshots por módulo)
- Generar 7 HTMLs estilo Mintlify (sidebar fijo, offline, con imágenes)
- Carpeta `docs/v1.0.0-beta/assets/img/` con naming descriptivo por pantalla

---

## ✅ SESIÓN XX+72 (2026-05-08) — Paridad web↔mobile dashboard padre: badge hora entrada + retardos del mes

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Corrección de paridad web↔mobile en el dashboard del padre. Mobile no mostraba el badge de hora de entrada ni el contador/alerta de retardos del mes que sí aparecen en web.

### Cambios

- **`mobile/app/(padre)/index.jsx`** — `HijoCard`: se agregó sección `entradaBox` entre el header del hijo y el banner de adeudo:
  - Badge `🚪 ✅ Entrada autorizada` + hora de entrada
  - Badge `⚠️ Retardo` cuando `es_retardo && puede_entrar`
  - Alerta amarilla acumulada (1-2 retardos) con contador `X/3 del mes`
  - Alerta roja (3+ retardos) con aviso de rechazo al día siguiente
  - Los datos ya venían del backend (`filtro_entrada`, `retardos_mes_total`) — solo faltaba renderizarlos

### Datos backend ya disponibles (sin cambios)

- `filtro_entrada.hora_entrada`, `filtro_entrada.es_retardo`, `filtro_entrada.puede_entrar`
- `retardos_mes_total` (subquery COUNT en `/alumnos/mis-hijos`)

---

## ✅ SESIÓN XX+71 (2026-05-08) — Validación QA final: Ítems 15, 16, 17-19, 20, 23, 27, 28

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Sesión de validación QA final de todos los ítems pendientes de las sesiones XX+63 al XX+66. Sin bugs nuevos — todos los ítems pasaron sin incidencias.
...
## ✅ SESIÓN XX+70 (2026-05-08) — QA mobile: Fix animo crash padre, FiltroSalida QR, Tab Salida web+mobile, nombre_quien_recoge

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Sesión de corrección de bugs QA detectados por Valeria al validar el flujo QR de salida y el dashboard padre mobile. Se implementó paridad completa web ↔ mobile para el registro de salida, y se corrigieron crashes y datos faltantes.

### Bugs corregidos

1. **"Animo doesn't exist" crash en dashboard maestra mobile** (`mobile/app/(maestra)/index.jsx:40`)
   - `i.valor`/`i.etiqueta` no existen en los items del catálogo → Fix: `i.key`/`i.emoji`

2. **"Animo doesn't exist" crash en dashboard padre mobile (HijoCard)** (`mobile/app/(padre)/index.jsx`)
   - `useCatalogo('animo/cuanto/comportamiento')` declarados en el componente padre, fuera del scope de `HijoCard` → Fix: mover los 3 `useCatalogo` dentro de `HijoCard`

3. **QR scan salida no mostraba FiltroSalida** (`mobile/app/(maestra)/qr-scanner.jsx`)
   - Solo mostraba botón "Confirmar". Fix: `ConfirmacionSalida` reescrito con selector de quién recoge (padres + autorizados + Otro), checklist sanitario completo (pañal, pertenencias, estado físico, entrega conforme), notas

4. **Padres y autorizados no aparecían en selector QR** (`backend/src/controllers/alumnosController.js`)
   - `buscarPorQR` no consultaba esas tablas. Fix: agregar queries con `p.parentesco AS tipo` y `nombre_completo AS nombre`

5. **`nombre_quien_recoge` vacío en BD para padre/autorizado** (`backend/src/routes/asistencia.js`)
   - El INSERT usaba el campo directo del body (solo llegaba para "Otro"). Fix: resolver el nombre antes del `BEGIN` consultando `padres` / `personas_autorizadas` según el tipo

6. **Tab Salida ausente en bitácora padre web** (`web/src/pages/padre/Bitacora.jsx`)
   - No existía el tab. Fix: agregado tab "👋 Salida" (8 tabs, grid `lg:grid-cols-8`), con hora, quién recogió, banner anticipada+motivo, checklist PildoraBool, notas

7. **Tab Salida ausente en bitácora padre mobile** (`mobile/app/(padre)/bitacora.jsx`)
   - No existía el tab ni el destructuring de `data?.salida`. Fix: tab "Salida" con icono `exit-outline`, destructuring `salidaMostrada`, contenido con `FilaInfo` + `PildoraBool` + banner anticipada + notas

8. **Chip de salida ausente en dashboard padre mobile (HijoCard)** (`mobile/app/(padre)/index.jsx`)
   - `filtro_salida` llegaba del backend pero no se renderizaba. Fix: chip verde "🚪 Salió a las HH:MM / Recogido por X" con badge naranja si es anticipada

9. **Backend mis-hijos sin nombre_quien_recoge** (`backend/src/routes/alumnos.js`)
   - `filtro_salida` solo incluía hora/anticipada/motivo. Fix: agregar `rs.nombre_quien_recoge` y `rs.recogido_por_tipo` al SELECT y al objeto devuelto

10. **Backend /bitacora/:alumnoId sin datos de salida** (`backend/src/routes/bitacora.js`)
    - No consultaba `registro_salida` ni `registro_salida_sanitario`. Fix: agregar ambas queries al `Promise.all` y devolver objeto `salida` combinado

### Archivos modificados

- `mobile/app/(maestra)/index.jsx` — Fix catálogo animo (i.key/i.emoji)
- `mobile/app/(maestra)/qr-scanner.jsx` — ConfirmacionSalida completa + estilos
- `mobile/app/(padre)/index.jsx` — HijoCard: useCatalogo dentro + chip salida + estilos
- `mobile/app/(padre)/bitacora.jsx` — Tab Salida agregado (tab + destructuring + contenido)
- `web/src/pages/padre/Bitacora.jsx` — Tab Salida agregado (tab + salidaMostrada + contenido)
- `backend/src/controllers/alumnosController.js` — buscarPorQR: queries padres + autorizados
- `backend/src/routes/alumnos.js` — mis-hijos: nombre_quien_recoge + recogido_por_tipo
- `backend/src/routes/bitacora.js` — queries registro_salida + registro_salida_sanitario
- `backend/src/routes/asistencia.js` — nombreFinal resuelto antes del INSERT

### Ítems QA validados ✅

- QR scan desde app maestra: FiltroSalida completo con opciones de quién recoge ✅
- Dashboard padre mobile: chip verde de salida con hora y nombre ✅
- Bitácora padre web: tab Salida con todos los datos ✅
- Bitácora padre mobile: tab Salida con todos los datos ✅

---

## ✅ SESIÓN XX+69 (2026-05-08) — Validación QA mobile: Dashboard maestra + Bitácora maestra

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Sesión de validación QA mobile con cuenta `prekinder@happyschool.edu.mx` en Expo Go. Se detectaron y corrigieron 5 bugs en bitácora maestra mobile y se implementó paridad de actividades del día (nivel grupo) con web.

### Ítem validado ✅

- **Dashboard maestra mobile** — Carga normal sin errores ✅

### Bugs corregidos en `mobile/app/(maestra)/bitacora.jsx`

1. **Badge "Pendiente"/"Guardada" siempre mostraba "Pendiente"** — Usaba `alumno.bitacora_hoy` (campo inexistente). Fix: usar `alumno.estado_animo` que sí viene en el endpoint `/grupos/mi-grupo`.

2. **"Comida extra" mostraba badge "Precargado del menú semanal"** — `menuPrecargado` era global para todos los tabs. Fix: agregar condición `tiempoActivo !== 'comida_extra'` en el badge.

3. **Actividades del día no aparecían en mobile** — No había sección para definirlas. Fix: se agregó panel "🎨 Actividades del día" en `SelectorAlumno` (nivel grupo, igual que web) con editor + fotos por actividad.

4. **Participación del alumno no se guardaba en web** — `parseInt(uuid)` devuelve `NaN`, el backend filtraba con `actividad_grupo_id &&` descartando todos los registros. Fix: enviar el UUID directo sin `parseInt`.

5. **Editor de actividades sin opción de foto** — Se agregó botón "📷 Agregar foto" por actividad (cámara/galería) con preview y opción de quitar, enviadas como multipart al guardar.

### Archivos modificados

- [mobile/app/(maestra)/bitacora.jsx](mobile/app/(maestra)/bitacora.jsx)

---

## ✅ SESIÓN XX+68 (2026-05-08) — Validación QA web + Fix FiltroSalida

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado

### Resumen

Sesión de validación QA de ítems acumulados (XX+63 al XX+66) en portal web. Dos fixes aplicados en FiltroSalida.

### Lo que se validó ✅

- **Calendario web padre** — Día de hoy resaltado correctamente
- **Dashboard padre web** — Carga instantánea al regresar (< 5 min, sin spinner)
- **Ítems 8-10** — Fechas en Tareas, Pagos, FiltroEntrada/Salida funcionan igual
...
- **Ítem 26** — FiltroSalida web: 1 sola vista sin botón "Siguiente" ✅

### Fixes aplicados en FiltroSalida.jsx

1. **Doble scroll eliminado** — Se quitó `overflow-y-auto max-h-[70vh] pr-1` del contenedor interno del ModalSalida. El Modal.jsx ya tiene `max-h-[90vh]` propio, el scroll anidado causaba doble scrollbar.
2. **Checklist marcado por default** — `panialLimpio`, `pertenenciasOk`, `estadoFisicoOk`, `entregaConforme` ahora inician en `true` para agilizar el registro de salida.

### Pendiente para próxima sesión (mobile)

- Ítem 12 — Bitácora maestra mobile
- Ítem 15 — Bitácora padre mobile: galería y cámara
- Ítem 16 — Navegar fechas en mobile
- Ítem 17-19, 20, 23-25, 27-29

### Archivos modificados

- [web/src/pages/maestra/FiltroSalida.jsx](web/src/pages/maestra/FiltroSalida.jsx)
- [PENDIENTES.md](PENDIENTES.md)

---

## ✅ SESIÓN XX+67 (2026-05-08) — Planificación pre-QA + 11 ítems documentación

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado — sin código, solo planificación

### Resumen

Sesión de planificación pre-QA/piloto con escuela real. No se escribió código.

### Lo que se hizo

- Análisis de qué hace falta antes de dar la app a una escuela para piloto
- Decisión de prioridad: documentación antes de deploy
- Agregados 11 ítems a `PENDIENTES.md` bajo nueva sección `📚 DOCUMENTACIÓN — Pre-QA / Piloto con escuela real`:
  1. Manual de usuario — Directora (portal web)
  2. Manual de usuario — Administrador (portal web)
  3. Manual de usuario — Maestra (web + mobile)
  4. Manual de usuario — Padre (web + mobile)
  5. Guía de onboarding — configurar escuela desde cero
  6. Checklist de alta de escuela — orden exacto de dependencias
  7. FAQ / Casos comunes de soporte
  8. Guía de variables de entorno (.env backend + mobile)
  9. Guía de migraciones en producción (51 migraciones)
  10. Plan de respaldo de base de datos
  11. Plan de rollback
...
## ✅ SESIÓN XX+66 (2026-05-08) — UX: reducir clics a máx. 2 — Ítems 26-29

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado — pendiente validación en browser/app por Valeria

### Resumen

4 mejoras de UX que reducen la fricción de acciones frecuentes. Objetivo: máx. 2 clics por acción.

### Lo que se hizo

| Ítem | Componente | Antes | Ahora |
|------|-----------|-------|-------|
| 26 | `FiltroSalida.jsx` (web) | Modal 2 pasos: "Siguiente" → "Confirmar Salida" (4 clics) | 1 sola vista con scroll, 1 botón "Confirmar Salida". Estado `paso` eliminado |
| 27 | `Bitacora.jsx` padre (web) | Solo chevrones día a día (5 clics para semana anterior) | Fila extra: "← Semana anterior" / "Semana siguiente →" (saltan 5 días hábiles) |
| 28 | `bitacora.jsx` padre (mobile) | 2 botones separados: "🖼 Galería" + "📷 Cámara" | 1 botón "📷 Adjuntar foto de receta" → Alert ActionSheet nativo (Galería/Cámara/Cancelar). Ambas ocurrencias actualizadas |
| 29 | `index.jsx` maestra (mobile) | QR en 3 caminos distintos (banners condicionales + botón en grid igual a los demás) | Botón rojo prominente "Escanear QR de alumno" con ancho completo, siempre visible, encima de acciones secundarias |

### Archivos modificados

- [web/src/pages/maestra/FiltroSalida.jsx](web/src/pages/maestra/FiltroSalida.jsx)
- [web/src/pages/padre/Bitacora.jsx](web/src/pages/padre/Bitacora.jsx)
- [mobile/app/(padre)/bitacora.jsx](mobile/app/(padre)/bitacora.jsx)
- [mobile/app/(maestra)/index.jsx](mobile/app/(maestra)/index.jsx)
...
- [ ] **Ítem 28** — Bitácora padre mobile: declarar medicamento con 1 botón de foto (ActionSheet)
- [ ] **Ítem 29** — Dashboard maestra mobile: 1 solo acceso claro al scanner QR (botón rojo prominente)
...
## ✅ SESIÓN XX+63 (2026-05-08) — UX + Performance: Audit completo + Limpieza código + Centralización utils (Parte 1)

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado — pendientes de validación en browser/app por Valeria

### Resumen

Audit exhaustivo de UX, performance y código duplicado en toda la app (web + mobile). Se identificaron 73 problemas. Esta sesión resuelve los de mayor impacto inmediato. Los pendientes restantes quedan documentados en PENDIENTES.md para la próxima sesión.

### Lo que se hizo

| Cambio | Archivos | Detalle |
|--------|----------|---------|
| **Eliminar módulo Chat** | `mobile/app/(padre)/chat.jsx` eliminado, `mobile/app/(padre)/_layout.jsx` | Chat cancelado definitivamente. Archivo stub eliminado, entrada del layout limpiada |
| **Bug fix Calendario padre web** | `web/src/pages/padre/Calendario.jsx:152` | `hoyStr` usaba `getMonth()` (0-11) sin padding — el día de hoy nunca se resaltaba correctamente. Reemplazado por comparación directa de números: `year === hoy.getFullYear() && month === hoy.getMonth() && d === hoy.getDate()` |
| **Typo getSemanaActual** | `mobile/app/(padre)/comida.jsx:58` | `getSemanActual` → `getSemanaActual` (faltaba la "a") |
| **Normalizar imports mobile** | 6 archivos: `calendario.jsx`, `asistencia.jsx`, `pagos.jsx`, `bitacora.jsx` (padre + maestra) | Imports `../../src/services/api` → `@/services/api` (y equivalentes). Alias `@/` ya estaba configurado en babel.config.js |
| **staleTime queries Dashboard padre** | `web/src/pages/padre/Dashboard.jsx`, `mobile/app/(padre)/index.jsx` | `mis-hijos` y `eventos-proximos` sin staleTime → `staleTime: 5 * 60 * 1000`. Evita refetch en cada navegación |
| **staleTime + refetchInterval maestra web** | `web/src/pages/maestra/Dashboard.jsx` | `mi-grupo`, `turno-hoy`, `estadisticas-hoy`, `tareas-hoy`, `alumnos-alerta` — staleTime alineado con refetchInterval (30s o 60s según query) |
| **Reducir refetchInterval maestra mobile** | `mobile/app/(maestra)/index.jsx` | `mi-grupo`: 30s→2min, `turno-hoy`: 60s→5min. Reduce consumo de batería y datos |
| **Centralizar esCumpleanos()** | `web/src/utils/asistencia.js` + 4 archivos consumidores | Función estaba duplicada 4 veces (Asistencia, Dashboard maestra, FiltroEntrada, Dashboard directora). Movida a `utils/asistencia.js` y reemplazada por import |

### Pendientes detectados para próximas sesiones

Ver sección "🧹 UX + PERFORMANCE — En progreso" en PENDIENTES.md ítems 8–29.

### Tu lista de validación

- [ ] **Calendario web padre** — El día de hoy aparece resaltado
- [ ] **Dashboard padre web** — No hace spinner al regresar si pasaron menos de 5 min
- [ ] **Dashboard maestra mobile** — Carga normal sin errores

---

## ✅ SESIÓN XX+62 (2026-05-08) — Portal Padre Mobile: Menores — ícono evento dinámico + fix tareas pendientes

**Fecha:** 2026-05-08 | **Estado:** ✅ Implementado — validación en Expo Go pendiente por Valeria

### Resumen

Corrección de los 2 items de `### 🟡 MENORES — Portal Padre Mobile` en PENDIENTES.md. Cambios en mobile y web (paridad).

### Lo que se hizo

| Cambio | Archivos | Detalle |
|--------|----------|---------|
| **Ícono evento dinámico en lista** | `mobile/app/(padre)/index.jsx` | La card de evento mostraba siempre `<Ionicons name="calendar-outline">` fijo. Ahora muestra `ev.categoria_icono` (emoji dinámico) si existe, con fallback al ícono genérico. Paridad con web que ya usaba `ev.categoria_icono \|\| '📅'`. |
| **Fix tareas pendientes — error silencioso** | `mobile/app/(padre)/index.jsx`, `web/src/pages/padre/Dashboard.jsx` | Root cause: `.catch(() => [])` dentro del `queryFn` silenciaba errores 403/400 → componente retornaba `null` sin aviso. Corregido: removido el catch del queryFn, usando `isError` de `useQuery` para mostrar mensaje "No se pudieron cargar las tareas" cuando el endpoint falla. Paridad web↔mobile. |

### Notas técnicas

- El `.catch(() => null)` en `pagos/estado` dentro de `HijoCard` se mantiene intencionalmente (ese sí debe silenciar fallos para no romper la card).
- El error de tareas suele ser 403 si el `authCheck` falla (`alumno_padre → padres → usuarios`). Con `isError` visible ahora se puede detectar en QA.

---

## ✅ SESIÓN XX+61 (2026-05-08) — Dashboard Maestra Mobile: UX/Diseño Paridad

**Fecha:** 2026-05-08 | **Estado:** ✅ Implementado — validación en Expo Go pendiente por Valeria

### Resumen

Implementación de los 4 items de `### 🟡 MENORES — UX/Diseño Maestra` en PENDIENTES.md. Solo mobile, no afecta web. Un único archivo modificado: `mobile/app/(maestra)/index.jsx`.

### Lo que se hizo

| Cambio | Detalle |
|--------|---------|
| **Campanita de notificaciones** | Importado `NotificationBell` desde `@/components/NotificationBell`. Agregado al header junto al logout. Misma UX que portal padre (bottom-sheet, badge conteo, modales urgentes). |
| **Turno de puerta** | Nueva query a `/turnos-puerta/hoy` (`refetchInterval: 60000`). Banner morado aparece cuando `turnoHoy?.tiene_turno === true`. Navega al QR scanner. |
| **Cumpleaños** | Función `esCumpleanos()` idéntica a web. Detecta desde `grupo.alumnos` sin endpoint extra. Banner amarillo + emoji 🎂 en tarjeta del alumno en lista. |
| **Rechazados por síntomas** | Detecta `puede_entrar === false` con síntomas desde `grupo.alumnos`. Banner rojo con lista y motivo. Misma lógica que web, sin endpoint extra. |
| **Confirmaciones de comida** | Nueva query a `/comida/confirmaciones` con semana actual. Muestra cuántos niños comen hoy (badge verde) + lista confirmados con modalidad (semana completa o días específicos). |

### Notas técnicas

- Acumulados en el mismo dashboard para no sobrecargar la pantalla: los banners son condicionales y solo aparecen cuando hay datos relevantes.
- `getLunesActual()` copiada de web para calcular el lunes de la semana en curso.
- `esCumpleanos()` usa la misma lógica de comparación mes/día que web (evita desfase de zona horaria).

---

## ✅ SESIÓN XX+60 (2026-05-08) — Bitácora Maestra Mobile: Paridad 🟠 IMPORTANTES

**Fecha:** 2026-05-08 | **Estado:** ✅ Implementado — validación en Expo Go pendiente por Valeria

### Resumen

Implementación de los 3 items marcados como 🟠 IMPORTANTES en PENDIENTES.md (diferencias detectadas en revisión Sesión XX+59). Solo mobile, no afecta web.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| **Alimentación: precarga del menú semanal** — Calcula `semanaLunes`, queries a `GET /comida/menu?semana=` y `GET /comida/confirmacion/:alumnoId?semana=`. En `useEffect`: si alumno tiene comida confirmada y hay menú publicado, precarga `que_comio` para desayuno/colación/comida filtrando por `nivelCodigo`. Badge "📋 Precargado del menú semanal" visible. | `mobile/app/(maestra)/bitacora.jsx` |
| **Medicamentos: fotos de receta y envase en recepción** — Estados `recFotoReceta` y `recFotoEnvase`. `pickFotoRecepcion(setter)` con Alert Cámara/Galería usando `base64:true`. Envía `foto_receta_base64` al backend. Previews de miniaturas en el form. `foto_envase` guardado en UX (backend sin soporte aún). | `mobile/app/(maestra)/bitacora.jsx` |
| **Actividades: fotos del alumno por actividad** — `subirFotoActividad()` con Alert Cámara/Galería (selección múltiple), POST multipart a `/bitacora/actividades/:id/fotos-alumno`. `eliminarFotoActividad()` con Alert de confirmación. Galería horizontal de miniaturas por actividad con botón ✕. `uploadingActId` para loading state individual. | `mobile/app/(maestra)/bitacora.jsx` |

### Notas técnicas

- `foto_envase`: backend no tiene columna en BD. La web también la envía y el backend la ignora. En mobile se guarda en state local para paridad de UX; cuando backend soporte `foto_envase_url`, solo hay que agregar `body.foto_envase_base64`.
- Dependencias `useEffect` actualizadas a `[bitacoraExistente, menuSemana, confirmacionComida]`.

---

## ✅ SESIÓN XX+59 (2026-05-08) — Bitácora Maestra Mobile: Paridad web completa

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado y validado (validación en browser pendiente por Valeria)

### Resumen

Implementación de 4 secciones de la Bitácora Maestra Mobile que existían en web pero estaban ausentes en mobile. Comparación línea a línea contra `web/src/pages/maestra/Bitacora.jsx` y corrección de 3 discrepancias detectadas. 3 gaps adicionales (fotos, menú precarga) documentados en PENDIENTES.md para implementación separada.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| **Alimentación: 4 tiempos** — Reemplazado campo único por estado `comidas` con tabs Desayuno / Colación / Comida / Comida Extra. Carga desde `data.comida[]`, guarda como array `[{tiempo, que_comio, cuanto_comio, observaciones}]` | `mobile/app/(maestra)/bitacora.jsx` |
| **Incidentes/Accidentes** — Sección con lista de incidentes del día (hora + descripción + `reportado_por_nombre`) + form emergente `POST /bitacora/incidente` | `mobile/app/(maestra)/bitacora.jsx` |
| **Actividades del día** — Lista desde `bitacoraExistente.actividades` (sin query extra), botones Sí / No / — por actividad, `POST /bitacora/actividades-alumno`, carga participación existente al entrar | `mobile/app/(maestra)/bitacora.jsx` |
| **Medicamentos completos** — `administrarRecepcionMutation` corregido a `{recepcionId, tomaId}`. Agregado form de administración directa (`medNombre`, `medDosis` → `POST /bitacora/medicamento`) | `mobile/app/(maestra)/bitacora.jsx` |
| **grupoId en navegación** — Pasado desde `SelectorAlumno` como param URL, recibido en `FormularioBitacora` | `mobile/app/(maestra)/bitacora.jsx` |
| **Nuevos estilos** — `tiempoTabs`, `tiempoTab`, `tiempoTabOn`, `tiempoTabTxt`, `tiempoTabTxtOn` | `mobile/app/(maestra)/bitacora.jsx` |

### Discrepancias corregidas en revisión de paridad

1. **Actividades — fuente de datos**: Primera implementación usaba query separado a `/bitacora/actividades-grupo`. Corrección: `bitacoraExistente.actividades` ya viene en la respuesta del bitacora (igual que web).
2. **Medicamentos — firma de mutación**: `administrarRecepcionMutation` llamaba con `rec.id` directo. Corrección: `{ recepcionId: rec.id, tomaId: toma?.id ?? null }` igual que web.
3. **Actividades — botón "—" ausente**: Solo había toggle Sí/No. Corrección: tercer botón "—" que hace `delete c[act.id]` para limpiar selección.

### Pendientes documentados en PENDIENTES.md (con especificación técnica)

- Precarga del menú semanal en campos de alimentación (`GET /comida/menu?semana=` + `GET /comida/confirmacion/:alumnoId?semana=`)
- Fotos de receta y envase en recepción de medicamento (multipart `foto_receta`, `foto_envase`)
- Fotos del alumno por actividad (`POST /bitacora/actividades/:actividadGrupoId/fotos-alumno`, multipart `fotos[]`)

---

## ✅ SESIÓN XX+58 (2026-05-09) — WhatsApp solo uno a uno: 14 plantillas + botón recordatorio en adeudos

**Fecha:** 2026-05-09 | **Estado:** ✅ Completado

### Resumen

Limpieza de plantillas WhatsApp para validación con Twilio: solo quedan activas las 14 plantillas uno a uno (padre específico de un alumno). Eliminadas las plantillas broadcast. Conectado botón 📲 en tabla de adeudos para enviar recordatorio de pago directamente al tutor del alumno con saldo pendiente.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| Migración 050: desactivar `aviso_nuevo` y `evento_nuevo` (broadcast) | `backend/migrations/050_desactivar_plantillas_broadcast.sql` |
| Migración 051: eliminar 9 plantillas (broadcast + sin disparador planificado) — quedan 14 activas todas uno a uno | `backend/migrations/051_eliminar_plantillas_no_usadas.sql` |
| Endpoint `POST /pagos/alumno/:alumnoId/recordatorio` — usa plantilla `recordatorio_pago`, obtiene tutor principal y saldo pendiente | `backend/src/routes/pagos.js` |
| Botón 📲 en columna Acción de tabla adeudos — solo visible si `saldo_pendiente > 0`, con confirmación antes de enviar | `web/src/pages/directora/Pagos.jsx` |

### 14 plantillas activas (todas uno a uno)

`sin_recoger`, `recordatorio_pago`, `suspension`, `recargo`, `retardo`, `no_entrada`, `fiebre`, `incidente`, `medicamento`, `alerta_salud`, `persona_no_autorizada`, `salida_anticipada`, `sin_comida`, `pago_comida_lunes`

### Pendiente validar con Twilio

- Botón 📲 en FiltroSalida → `sin_recoger` (después 15:00, alumno Camila, tel 9932160007)
- Botón 📲 en tabla adeudos → `recordatorio_pago` (padre con saldo pendiente)

---

## ✅ SESIÓN XX+57 (2026-05-09) — Plantillas WhatsApp sin disparador implementadas

**Fecha:** 2026-05-09 | **Estado:** ✅ Completado y validado

### Resumen

Implementación de las 10 funciones `notificar*` faltantes en `whatsappService.js`. Prueba directa 10/10 vía API Twilio (límite sandbox alcanzado, no error de código). Conexión de 3 disparadores en módulos existentes. Botón 📲 en FiltroSalida con condición de hora.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| 10 funciones `notificar*` nuevas: `recordatorio_pago`, `recargo`, `evento_nuevo`, `boleta_lista`, `sin_recoger`, `documentos_pendientes`, `encuesta_nueva`, `aviso_nuevo`, `suspension`, `pago_comida_lunes` | `backend/src/services/whatsappService.js` |
| Endpoint `POST /api/asistencia/alerta-sin-recoger` — envía WA a todos los padres del alumno | `backend/src/routes/asistencia.js` |
| Disparador `aviso_nuevo` conectado al `POST /aviso-extraordinario` (fire-and-forget, no bloquea respuesta) | `backend/src/routes/notificaciones.js` |
| Disparador `suspension` conectado al `POST /:id/historial-servicios` con `accion: 'baja'` | `backend/src/controllers/alumnosController.js` |
| Botón 📲 en tarjeta de alumno pendiente de recoger — visible solo después de hora de salida configurada | `web/src/pages/maestra/FiltroSalida.jsx` |
| Número de prueba actualizado a `9932160007` para `mama.camila@happyschool.edu.mx` | BD — tabla `padres` |

### Validado
- 10/10 plantillas probadas directamente vía Twilio API — todas `enviado` con SID confirmado
- Límite diario sandbox (50 msg) alcanzado — no es error de código, se resetea cada día
- Botón 📲 oculto antes de hora de salida, visible después de `hora_salida_normal` configurada
- Plantillas inactivas respetadas — sistema no envía si `activa = false` en BD

### Pendiente validar mañana
- Botón 📲 en FiltroSalida después de las 15:00 con alumno presente (Camila Torres García)
- Confirmar llegada del mensaje al 9932160007

---

## ✅ SESIÓN XX+56 (2026-05-08) — WhatsApp Sandbox validado end-to-end

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado y validado

### Resumen

Diagnóstico y resolución del error 63015 de Twilio Sandbox. Fix de normalización de números mexicanos en WhatsApp (+521 intermedio). Ruta pública para recibo PDF. Botón Email quitado del modal de pase temporal.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| Fix: números MX normalizados a `+521XXXXXXXXXX` (WhatsApp registra móviles con `1` intermedio) | `backend/src/services/whatsappService.js` |
| Ruta pública `GET /api/pagos/:id/recibo-publico` sin JWT (UUID como secreto) | `backend/src/routes/pagos.js` |
| Link en mensaje WhatsApp apunta a `/recibo-publico` (accesible desde celular del padre) | `backend/src/routes/pagos.js` |
| Variable `APP_BASE_URL` agregada al .env para producción | `backend/.env` |
| Botón ✉️ Email eliminado del modal de pase temporal QR | `web/src/pages/padre/Dashboard.jsx` |

### Validado
- Mensaje WhatsApp llega al celular del padre con recibo de pago
- Link del recibo abre PDF sin requerir login (200 sin token)
- Ruta protegida `/recibo` sigue requiriendo auth (401 sin token)
- QR temporal: modal muestra solo Descargar + WhatsApp (sin Email)

---

## ✅ SESIÓN XX+55 (2026-05-08) — WhatsApp feature flag + Twilio Sandbox

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado (prueba sandbox pendiente número ajeno)

### Resumen

Implementación completa de feature flag WhatsApp (`WHATSAPP_ENABLED`) para permitir activar/desactivar Twilio sin tocar código. Configuración de cuenta Twilio Sandbox real. Bug fix en query de pagos que usaba `telefono` en lugar de `telefono_whatsapp`.

### Lo que se hizo

| Cambio | Archivo(s) |
|--------|-----------|
| Endpoint `GET /api/config/whatsapp` → `{ enabled: bool }` | `backend/src/routes/config.js` |
| Store Zustand `appConfigStore` con `whatsappEnabled` + `cargarConfig()` | `web/src/store/appConfigStore.js` |
| `PrivateRoute` carga config al autenticarse | `web/src/App.jsx` |
| Botón "Enviar por WhatsApp" oculto si disabled | `web/src/pages/directora/Pagos.jsx` |
| Botón "📱 WhatsApp" QR temporal oculto si disabled | `web/src/pages/padre/Dashboard.jsx` |
| Tab "Plantillas WA" oculto si disabled | `web/src/pages/directora/Configuracion.jsx` |
| Botón WhatsApp en QR padre oculto si disabled (useQuery) | `mobile/app/(padre)/qr.jsx` |
| Credenciales Twilio Sandbox reales en .env | `backend/.env` |
| Fix: query usaba `t.telefono` → `COALESCE(t.telefono_whatsapp, t.telefono)` | `backend/src/routes/pagos.js` |
| Fix: normalización número `+52` no duplicado | `backend/src/services/whatsappService.js` |

### Estado Twilio Sandbox
- Cuenta: `ACea7649c9a3b00470ad948db94532e4ef`
- Sandbox activo, código: `join although-previous` → `+14155238886`
- Limitación: el número propietario de la cuenta no puede ser destinatario del sandbox
- Pendiente: probar con número ajeno a la cuenta Twilio

---

## ✅ SESIÓN XX+54 cont. (2026-05-08) — Mobile: bugs asistencia, bitácora, tareas, backend QR

**Fecha:** 2026-05-08 | **Estado:** ✅ Validado por Valeria

### Resumen

Continuación de sesión XX+54. Se resolvieron todos los bugs críticos e importantes pendientes: crash en asistencia, error de transacción PostgreSQL en salida QR, retardo no visible, alumnos sin asistencia en bitácora, baño condicional por pañal, ánimo sin descripción, salida sanitaria en formulario, nombre Miss truncado, DatePicker en tareas. También se eliminó Galería de todos los lugares restantes (dashboard mobile + web nav + web dashboard).

### Bugs corregidos

| Bug | Archivo(s) | Fix |
|-----|-----------|-----|
| Crash asistencia: `ESTADO_CONFIG.pendiente` undefined | `mobile/src/constants/asistencia.js` | Agregado `pendiente: { label: 'Pendiente', color: '#718096', bg: '#EDF2F7', icon: '⬜' }` |
| Error transacción PostgreSQL en salida QR (sanitario) | `backend/src/routes/asistencia.js` | `SAVEPOINT sp_sanitario` alrededor del INSERT sanitario |
| Error transacción PostgreSQL en salida QR (pagos) | `backend/src/routes/asistencia.js` | `SAVEPOINT sp_pago_tardio` alrededor del INSERT pagos tardíos |
| Retardo Emilio Vega no visible en asistencia | `mobile/app/(maestra)/asistencia.jsx` | `estadoAlumno()` usaba `a.asistencia_hoy.estado` (inexistente) → corregido a `a.estado_asistencia`. `TarjetaAlumno` usa `alumno.hora_entrada`, `alumno.es_retardo`, `alumno.temperatura` directamente |
| Alumnos sin asistencia aparecían en bitácora | `mobile/app/(maestra)/bitacora.jsx` | `queryKey` incluye `fecha`, query pasa `?fecha=${fecha}`. Filtro: `['presente','retardo'].includes(a.estado_asistencia)` |
| Baño aparecía en alumnos con pañal | `mobile/app/(maestra)/bitacora.jsx` | Sección Baño envuelta en `{!usaPanial && (...)}` |
| Ánimo sin descripción bajo cada emoji | `mobile/app/(maestra)/bitacora.jsx` | Agregada `<Text style={animoLabel}>` con etiqueta de texto bajo cada emoji |
| Salida sanitaria en formulario captura | `mobile/app/(maestra)/bitacora.jsx` | Sección eliminada del formulario (solo se muestra al registrar salida) |
| Nombre Miss truncado en dashboard | `mobile/app/(maestra)/index.jsx` | `numberOfLines={2}` + `adjustsFontSizeToFit minimumFontScale={0.75}` + `marginRight: 12` |
| DatePicker no nativo en nueva tarea | `mobile/app/(maestra)/tareas.jsx` | `DateTimePicker` de `@react-native-community/datetimepicker` — reemplaza TextInput para fecha. Instalado con `--legacy-peer-deps` |
| Galería en dashboard mobile (acciones rápidas) | `mobile/app/(maestra)/index.jsx` | Eliminada entrada de Galería del array `ACCIONES` |
| Galería en nav web Maestra | `web/src/layouts/MaestraLayout.jsx` | Eliminado `{ to: '/maestra/galeria', icon: Image, label: 'Galería' }` de NAV_ITEMS |
| Galería en dashboard web Maestra | `web/src/pages/maestra/Dashboard.jsx` | Eliminada sección completa "Acciones rápidas" (solo contenía link a Galería) |

### Archivos modificados

- `mobile/src/constants/asistencia.js`
- `backend/src/routes/asistencia.js`
- `mobile/app/(maestra)/asistencia.jsx`
- `mobile/app/(maestra)/bitacora.jsx`
- `mobile/app/(maestra)/tareas.jsx`
- `mobile/app/(maestra)/index.jsx`
- `web/src/layouts/MaestraLayout.jsx`
- `web/src/pages/maestra/Dashboard.jsx`
- `mobile/package.json` (nueva dependencia DateTimePicker)
- `PENDIENTES.md`

### Pendiente validar (manual)

- QR normal scan desde app de la Miss (Valeria pendiente de validar)

---

## ✅ SESIÓN XX+54 (2026-05-07) — Mobile: colores rol, navegación fechas, fixes UI

**Fecha:** 2026-05-07 | **Estado:** ✅ Validado por Valeria

### Resumen

Sesión de revisión y corrección del portal Mobile (Maestra + Padre) basada en observaciones extensas de Valeria. Se implementaron 6 fixes y se documentaron 25+ issues pendientes para próximas sesiones.

### Cambios implementados

| Fix | Archivo(s) | Descripción |
|-----|-----------|-------------|
| Color tab bar Maestra: purple → green | `(maestra)/_layout.jsx` | `ACTIVE_COLOR: #805AD5 → #38A169`, borde tab `#C6F6D5`. Paridad con sidebar web (Maestra = verde) |
| Fondo bitácora Padre: rojo claro → blanco | `(padre)/bitacora.jsx` | 4 estilos con `#FFF8F8` → `COLORS.white` (header, cicloRow, fechaRow, animoHero) |
| Navegador de fechas — Bitácora Maestra | `(maestra)/bitacora.jsx` | Componente `SelectorFechaMaestra` con ‹/› arrows, salta fines de semana, bloquea fechas futuras. Fecha pasa como URL param al formulario |
| Navegador de fechas — Asistencia Maestra | `(maestra)/asistencia.jsx` | Componente `SelectorFecha` con mismo patrón. `fecha` pasa a query `/grupos/mi-grupo?fecha=` y a queryKey |
| Galería eliminada del tab bar Maestra | `(maestra)/_layout.jsx` | `options={{ href: null }}` — oculta tab, mantiene ruta accesible |
| ImagePicker sin crop forzado — Tareas | `(maestra)/tareas.jsx` | `allowsEditing: false`, sin `aspect`, `quality: 0.8`. Elimina el paso de recorte no deseado |
| Botones "Pub"/"Del" → texto completo | `(maestra)/tareas.jsx` | Texto: "Publicar" / "Eliminar", `smallBtn.minWidth: 72` para evitar truncamiento |

### Decisión de diseño — Color de rol en mobile

- **Scope correcto**: el color de rol (maestra=verde, padre=rojo, etc.) solo aplica al menú/tab bar de navegación, igual que en web donde solo colorea el sidebar activo
- **NO se propaga** a colores internos de pantallas (botones, spinners, títulos de sección) — estos usan sus propios colores semánticos
- **Web reference**: `accentColor` en `AppShell.jsx` y layouts por rol solo afecta nav activa, no contenido de páginas

### Issues analizados y documentados (pendientes próximas sesiones)

Ver PENDIENTES.md → sección "📱 MOBILE — Portal Maestra — Correcciones pendientes (Sesión XX+54)"

- 🔴 2 críticos (crash asistencia, error transacción salida QR)
- 🟠 7 importantes (paridad bitácora: alimentación por tiempos, medicamentos completos, incidentes, actividades, baño condicional, ánimo con etiqueta, salida sanitaria)
- 🟡 9 menores UX (turno de puerta, confirmaciones comida, campanita, DatePicker, nombre truncado, eventos padre, tarea pendiente padre)

### Archivos modificados

- `mobile/app/(maestra)/_layout.jsx`
- `mobile/app/(maestra)/bitacora.jsx`
- `mobile/app/(maestra)/asistencia.jsx`
- `mobile/app/(maestra)/tareas.jsx`
- `mobile/app/(padre)/bitacora.jsx`
...
## ✅ SESIÓN XX+52 (2026-05-07) — Validación QR Temporal Mobile + Fix flujo salida scanner

**Fecha:** 2026-05-07 | **Estado:** ✅ Validado en Expo Go (pase vencido pendiente mañana)

### Resumen

Validación completa del módulo QR Temporal en mobile. Se encontraron y corrigieron 4 bugs durante la sesión. El flujo completo padre→maestra funciona en Expo Go.

### Validaciones completadas

| Validación | Resultado |
|---|---|
| Sección "🔐 Pase temporal" aparece en pantalla QR del padre | ✅ |
| Padre genera pase temporal ingresando nombre del autorizado | ✅ |
| Miss escanea QR temporal → banner amarillo + nombre autorizado | ✅ |
| Pase cancelado → "Pase temporal inválido — fue cancelado por el padre" | ✅ |
| Pase vencido | Pendiente validar mañana (lógica implementada y probada en backend) |

### Bugs corregidos

| Bug | Fix |
|---|---|
| `expo-file-system` API legacy deprecated (`downloadAsync`, `writeAsStringAsync`, `EncodingType`) | Migrado a nueva API: `File`, `Paths`, `file.write()`, `File.downloadFileAsync()` |
| Botón "Descargar QR" solo mostraba compartir WhatsApp | Implementado `expo-media-library` + permisos en `app.json` (funciona en build nativo) |
| QR temporal vencía antes de medianoche en México | Fix: `toLocaleDateString('en-CA', { timeZone: 'America/Mexico_City' })` en lugar de `toISOString().slice(0,10)` (UTC) |
| Scanner mobile en modo salida se quedaba en blanco al detectar alumno | Agregado componente `ConfirmacionSalida` + `registrarSalidaMutation` + bloque de render para modo salida |
| `onError` del scanner mostraba "QR no reconocido" para cualquier error | Ahora muestra el mensaje real del backend con título diferenciado para temporales |

### Archivos modificados

| Archivo | Cambio |
|---|---|
| `mobile/app/(padre)/qr.jsx` | Nueva API expo-file-system, expo-media-library, handleDescargar completo |
| `mobile/app/(maestra)/qr-scanner.jsx` | ConfirmacionSalida component, registrarSalidaMutation, onError mejorado |
| `backend/src/controllers/alumnosController.js` | Fix fecha México en validación vigencia QR temporal |
| `mobile/app.json` | Permisos READ_MEDIA_IMAGES, READ_MEDIA_VIDEO + plugin expo-media-library |

### Pendiente

- Botón "Descargar QR" — implementado, requiere build nativo (expo-media-library no funciona en Expo Go)
- Validar pase vencido mañana en Expo Go

---

## ✅ SESIÓN XX+51 (2026-05-07) — Validación Notificaciones Push Mobile + Fix modal Entendido

**Fecha:** 2026-05-07 | **Estado:** ✅ Validado en Expo Go

### Resumen

Validación completa de notificaciones push en tiempo real en mobile (Expo Go). Se resolvió el error `java.io.IOException` al conectarse desde el celular (Metro bundler no estaba corriendo). Se arregló bug visual del botón "Entendido" en el modal urgente.

### Validaciones completadas en Expo Go

| Validación | Resultado |
|---|---|
| Badge sube en ≤15s al llegar aviso extraordinario | ✅ |
| "Entendido" marca como leída y badge baja | ✅ |
| 2 urgentes seguidas aparecen en cola (no simultáneas) | ✅ |
| Misma notificación NO aparece dos veces en sesión | ✅ |

### Bug corregido

**Botón "Entendido" cortado** — El truco `marginHorizontal: -28` para compensar el padding de la card no funcionaba en todos los dispositivos (mitad naranja, mitad blanco). Solución: separar el contenido en `View content` con padding y el botón directamente en la card con `overflow: 'hidden'`, eliminando el negative margin.

### Archivos modificados

| Archivo | Cambio |
|---|---|
| `mobile/src/components/NotificacionModalUrgente.jsx` | Fix layout botón: `overflow:'hidden'` en card + `View content` separado |

### Pendiente (requiere build nativo)

- Modal urgente apareciendo automáticamente en background vía push real (no funciona en Expo Go por limitación de `setNotificationHandler`)

---

## ✅ SESIÓN XX+50 (2026-05-07) — QR Temporal — Pase invitado para padre

**Fecha:** 2026-05-07 | **Estado:** ✅ Web validado — pendiente validación mobile en Expo Go

### Resumen

Implementación completa de QR temporal (pase invitado de 1 día) para que el padre autorice a una persona extraordinaria a recoger/dejar a su hijo. Solo disponible cuando la directora ya generó el QR permanente del alumno.

### Reglas de negocio

- 1 QR temporal activo por alumno por día (se invalida a medianoche)
- Cancelable por el padre en cualquier momento
- Solo para casos extraordinarios — las personas del círculo de confianza registradas usan el QR permanente
- Válido para entrada Y salida
- El padre ingresa el nombre de quien va a usar el QR
- La Miss ve banner amarillo diferenciado al escanear: "PASE TEMPORAL — Verificar identidad" con nombre del autorizado y padre que lo emitió

### Archivos creados/modificados

| Archivo | Cambio |
|---|---|
| `BD: tabla qr_temporales` | Creada — token único, nombre_autorizado, fecha_vigencia, cancelado |
| `backend/src/routes/alumnos.js` | 3 endpoints nuevos: GET/POST/DELETE /alumnos/:id/qr-temporal |
| `backend/src/controllers/alumnosController.js` | `buscarPorQR` actualizado — detecta HAPPYSCHOOL:TEMP:token |
| `web/src/pages/padre/Dashboard.jsx` | Componente `QRTemporalCard` — genera, muestra, cancela, comparte (descargar/WhatsApp/email) |
| `mobile/app/(padre)/qr.jsx` | `QRTemporalSection` — mismo flujo completo con modal nativo RN |
| `mobile/app/(maestra)/qr-scanner.jsx` | Acepta HAPPYSCHOOL:TEMP:, muestra banner amarillo al escanear |

### Funcionalidades

- Padre genera pase temporal ingresando nombre del autorizado (1 por día)
- QR se muestra con opciones: Descargar / WhatsApp / Email
- Padre puede cancelar el pase activo en cualquier momento
- Backend valida: vigencia del día, cancelación, token único
- Miss ve diferencia visual clara al escanear QR temporal vs permanente

---

## ✅ SESIÓN XX+49 (2026-05-07) — Push Notifications via Expo Push Service

**Fecha:** 2026-05-07 | **Estado:** ✅ Implementado — listo para cuando se genere APK/development build

### Resumen

Implementación completa de notificaciones push nativas (app cerrada) usando Expo Push Service — sin Firebase, sin tarjeta de crédito, costo $0. Reemplaza el plan original de Firebase FCM.

### Archivos creados/modificados

| Archivo | Cambio |
|---|---|
| `backend/src/services/pushService.js` | Reemplazado — usa fetch a Expo Push API, sin firebase-admin |
| `mobile/src/store/authStore.js` | Actualizado — obtiene ExponentPushToken directamente |
| `mobile/app.json` | Limpiado — removido googleServicesFile, agregado POST_NOTIFICATIONS |

### Funcionalidades

- `pushService.js` centraliza el envío: obtiene tokens de BD, filtra por `notificaciones_modal_tipos`, llama `exp.host/--/api/v2/push/send`
- Cache de tipos urgentes (5 min) para no consultar BD en cada push
- `POST /notificaciones/registrar-token` guarda `ExponentPushToken[xxx]` en `usuarios.fcm_token`
- Push integrado en 7 puntos: aviso extraordinario, alerta pago, vómito, diarrea, bitácora lista, medicamento (×3), incidente, entrada rechazada, salida anticipada, tarea nueva, tarea cancelada, toallitas, pañales, recordatorio medicamento (maestra)
- Filtro inteligente: solo tipos marcados como urgentes por la Directora disparan push nativa

### Notas técnicas

- En Expo Go estándar el token se registra pero la entrega push no funciona (limitación de Expo Go)
- Funciona al 100% en development build o APK generado con `eas build`
- No requiere configuración externa — cero credenciales adicionales
...
## ✅ SESIÓN XX+47 (2026-05-07) — Panel Plantillas WhatsApp editable (Directora)

**Fecha:** 2026-05-07 | **Estado:** ✅ Completada y validada en browser

### Resumen

Panel para que la Directora edite el texto de las 20 plantillas de WhatsApp automático desde el portal web, sin tocar código. Nuevo tab "Plantillas WA" en Configuración.

### Archivos creados/modificados

| Archivo | Cambio |
|---|---|
| `backend/src/routes/plantillas.js` | NUEVO — CRUD completo |
| `backend/src/routes/index.js` | Registro de ruta `/plantillas` |
| `web/src/components/directora/PlantillasWhatsApp.jsx` | NUEVO — UI completa |
| `web/src/pages/directora/Configuracion.jsx` | Nuevo tab "Plantillas WA" |

### Funcionalidades

**Backend `/api/plantillas`:**
- `GET /` — Lista las 20 plantillas (solo directora)
- `GET /:id` — Obtener una por ID
- `PUT /:id` — Editar nombre + texto (clave inmutable por diseño)
- `PATCH /:id/toggle` — Activar/desactivar

**Web — componente `PlantillasWhatsApp`:**
- Stats en cards (total / activas / inactivas)
- Búsqueda por nombre o clave
- Filtro Todas / Activas / Inactivas
- Vista expandible inline del texto completo
- Modal de edición con detección automática de variables `{{}}`
- Toggle activar/desactivar por plantilla

### Notas técnicas

- La `clave` de cada plantilla es inmutable (está atada al código de envío en cada módulo)
- Las variables `{{nombre}}`, `{{monto}}`, etc. se detectan automáticamente en el modal de edición
- Mobile sin cambios — feature exclusiva del portal Directora

---

## ✅ SESIÓN XX+46 (2026-05-07) — Cloudinary: Validación browser completa — todos los flujos

**Fecha:** 2026-05-07 | **Estado:** ✅ Completada

### Resumen

Validación manual en browser de los 5 flujos Cloudinary implementados en sesiones previas (XX+44 y XX+45). Todos los endpoints confirmados funcionales con curl y validados visualmente en browser por Valeria.

### Flujos validados

1. **Foto del día bitácora** — maestra sube foto → padre la ve en su portal (web + mobile)
2. **Fotos alumno por actividad** — maestra sube fotos del alumno haciendo actividad → visible en bitácora maestra (web)
3. **Vista padre fotos alumno por actividad** — padre ve galería de fotos de su hijo por actividad (web + mobile)
4. **Foto visitante** — directora registra visitante con foto → guardada en Cloudinary (web)
5. **Foto tarea** — miss crea tarea con foto adjunta → guardada en Cloudinary (web)

### Resultado curl

- Todos los endpoints retornan URL Cloudinary válida
- Todas las URLs de Cloudinary responden HTTP 200
- Foto bitácora persiste en BD (`foto_url` en `bitacora_diaria`)
- Fotos actividad persisten en BD (`actividades_fotos`)
- Flujo padre: `GET /api/bitacora/:alumnoId` incluye `foto_url` y `fotos_alumno[]`

---

## ✅ SESIÓN XX+45 (2026-05-07) — Cloudinary: Foto bitácora del día + Fotos alumno por actividad

**Fecha:** 2026-05-07 | **Estado:** ✅ Completada y validada en browser (2026-05-07)
...
## ✅ SESIÓN XX+65 (2026-05-08) — UX + Performance: Limpieza código muerto + Fixes defensivos + Consolidación lógica duplicada — Ítems 12, 17-25

**Fecha:** 2026-05-08 | **Estado:** ✅ Completado — pendientes de validación en browser/app por Valeria

### Resumen

Continuación del audit UX+Performance iniciado en XX+63. Eliminación de variables muertas, guards defensivos, centralización de helpers UI de Bitácora, y consolidación de lógica duplicada.

### Archivos nuevos creados

| Archivo | Contenido |
|---------|-----------|
| `web/src/components/ui/BitacoraHelpers.jsx` | `Seccion`, `FilaInfo`, `PildoraBool` helpers UI bitácoras web (Tailwind) |
| `mobile/src/components/BitacoraHelpers.jsx` | `Seccion`, `FilaInfo`, `PildoraBool` helpers UI bitácoras mobile (RN, estilos override) |

### Lo que se hizo

| Item | Cambio | Archivos afectados |
|------|--------|--------------------|
| 12 | Seccion, FilaInfo, PildoraBool centralizados en BitacoraHelpers | web/mobile: Bitacora.jsx maestra + padre |
| 17 | EMOJIS_COMIDA eliminada (nunca usada) | web/src/pages/padre/Dashboard.jsx |
| 18 | qrRef eliminado FiltroSalida + FiltroEntrada (bonus) | FiltroSalida.jsx + FiltroEntrada.jsx |
| 19 | ultimoAlumnoIdRef eliminado (asignado pero nunca leido) | web/src/pages/maestra/FiltroEntrada.jsx |
| 20 | Guard ev.fecha_inicio en ModalEvento + filter antes del map | mobile/app/(padre)/index.jsx |
| 21 | Ya tenia guard result.canceled - sin cambios | mobile/app/(padre)/bitacora.jsx |
| 22 | Ya tenia guards alumnoId suficientes - sin cambios | web/src/pages/padre/ComidaSemanal.jsx |
| 23 | SeccionMedicamentos extraida (~130 lineas x2 a 1 componente 2 usos) | web/src/pages/padre/Bitacora.jsx |
| 24 | ModalNuevaTarea + ModalEditarTarea (~244 lineas) a ModalTarea con mode | web/src/pages/maestra/Tareas.jsx |
| 25 | alumnos.map() a FlatList con scrollEnabled=false (virtualizacion) | mobile/app/(maestra)/index.jsx |

BUGS HISTÃ“RICOS â€” NUNCA REPETIR
