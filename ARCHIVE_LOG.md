...
### Parte B — Mobile (misma sesión)

**Archivos:** 4 | **Cambios:** 8 | **Commit:** `9116960`

| Cambio | Archivo | Tipo |
|--------|---------|------|
| Bug fecha `T00:00:00` → `T12:00:00` | `(maestra)/tareas.jsx` | Bug crítico |
| Bug `new Date(iso)` → anclar `T12:00:00` | `(padre)/calendario.jsx` | Bug crítico |
| `EMOJIS_ANIMO` hardcodeado → `useCatalogo('animo')` | `(maestra)/index.jsx` | Paridad web |
| Saludo dinámico `saludoHora()` | `(maestra)/index.jsx` | Paridad web |
| Saludo dinámico `saludoHora()` + `TRATAMIENTO_PARENTESCO` | `(padre)/index.jsx` | Paridad web |
| `📆` / `🗓️` → `📅` en eventos | `(padre)/index.jsx` | Paridad web |
| Emoji `🔄` → `ActivityIndicator` color `#E53E3E` | `(padre)/index.jsx` | Paridad web |
| Bug preexistente `tarea Titulo` → `tareaTitulo` | `(padre)/index.jsx` | Fix compilación |

**Nota sobre mobile:** La estructura está en `mobile/app/(padre)/` y `mobile/app/(maestra)/` (Expo Router), NO en `mobile/src/pages/`. El error de búsqueda inicial asumió estructura web — corregido.
